...
class AzkabanHelper {
  ...
  static void configureTask(AzkabanProject azkProject) {
    ...
    if (!azkProject.azkabanUserName) {
      ...
      logger.lifecycle("To avoid entering username: Refer https://iwww.corp.linkedin.com/wiki/cf/display/GRID/Hadoop+Gradle+Plugin+User+Guide#HadoopGradlePluginUserGuide-ConfigureAzkabanUploadInformation");
      //why don't we write it to .azkabanPlugin.json after upload is successuful
    }

    if (!azkProject.azkabanProjName) {
      azkProject.azkabanProjName = console.readLine("\nAzkaban project name: ");
      logger.lifecycle("To avoid entering project name: Refer https://iwww.corp.linkedin.com/wiki/cf/display/GRID/Hadoop+Gradle+Plugin+User+Guide#HadoopGradlePluginUserGuide-ConfigureAzkabanUploadInformation");
      //why don't we write it to .azkabanPlugin.json after upload is successuful
    }
  }
}
