...
/**
 * AzkabanKillFlowTask cancels a running flow execution in Azkaban.
 */
class AzkabanKillFlowTask extends DefaultTask {
  ...
  void killer() {
    ...
    killRunningAzkabanFlow(AzkabanHelper.readSession());
  }

  /**
   * Kills running flows in Azkaban.
   *
   * @param sessionId The Azkaban session id. If this is null, an attempt will be made to login to Azkaban.
   */
  void killRunningAzkabanFlow(String sessionId) {
    ...
    try {
      ...
      if (response.toLowerCase().contains("error")) {
        ...
        if (response.toLowerCase().contains("session")) {
          ...
          killRunningAzkabanFlow(null);
        } else {
          // If response contains other than session error
          logger.error("Fetching flows from ${azkProject.azkabanUrl} failed. Reason: " + new JSONObject(response).get("error").toString());
        }
        return;
      }
...
//                String flowEndTime = AzkabanHelper.epochToDate(jobsObject.get("endTime").toString());
        ...
        if (!runningExecSet.isEmpty()) {
          ...
          runningExecSet.each { execId ->
            uriList.add(new URIBuilder(azkProject.azkabanUrl)
                .setPath("/executor")
                .addParameter("session.id", sessionId)
                .addParameter("ajax", "cancelFlow")
                .addParameter("execid", execId)
                .build());
          }
          ...
          logger.lifecycle("Successfully killed flow execution(s)");
        } else {
