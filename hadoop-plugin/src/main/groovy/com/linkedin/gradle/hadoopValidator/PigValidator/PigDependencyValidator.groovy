        ...
        udfs.each { dep ->
          ...
          if (dep.pathType == Dependency.PathType.HDFS) {
            ...
            if(clusterURI == null){
              ...
              if(properties.containsKey("NameNode")){
            ...
            if(repo_url==null){
              if(properties.containsKey("repositoryURL")){
                ...
                throw new GradleException("Please specify ivy repository url in ${project.projectDir}/.hadoopValidatorProperties. Needed for checking artifact $path at line<${dep.lineno}>")
              }
            }
            ...
            String url = repo_url + (repo_url[-1] == '/' ? '' : '/') + udf_groupID + '/' +
                udf_name + '/' + udf_version + '/' + "${udf_name}-${udf_version}.jar"
            try {
            ...
            project.logger.warn("path $path at line<${dep.lineno}> includes keyword 'file:' which is used to denote absolute file path. It may not be the intended path")
            ...
            zipContents.each{
              if(_flag != 2 && it.name == filename){
                ...
                if(matcher.find()) {
                  alt_reg_command = dep.reg_command[0..matcher.start()-1]+"$it"+dep.reg_command.substring(matcher.end())
                }else {
            ...
            if (_flag==1){
              project.logger.warn("The dependency $path at line<${dep.lineno}> doesnot refer to a jar in libPath $libpath. It may not be the intended path")
            }
            else if(_flag==0) {
        ...
        if (state == 0) {
          ...
          while (matcher.find()) {
            ...
            if (fileName.length() > 7 && fileName[0..6] == "hdfs://") {
              //String alternate_reg_command = reg_command.replaceFirst(Pattern.quote("hdfs://"),Pattern.quote("webhdfs://"))
              //alternate_reg_command = alternate_reg_command.replaceFirst(/\:[0-9]+/,":50070")
              //fileText.replaceAll(Pattern.quote(reg_command),Pattern.quote(alternate_reg_command))
